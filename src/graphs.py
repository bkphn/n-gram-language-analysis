...
def draw_graph(FRAME_WIDTH, FRAME_HEIGHT, similarity_matrix_np, languages_list, THRESHOLD, FORM):
    ...
    plt.savefig(os.path.join(FIG_DIR, "graph.pdf"), format="pdf", bbox_inches="tight")
    print("Pomyślnie wygenerowano plik: graph.pdf")

    plt.show()

def draw_heatmap(similarity_matrix_np, languages_list):
    sns.set_theme(style="white")
    fig, ax = plt.subplots(figsize=(14, 12))
    labels = [lang.capitalize() for lang in languages_list]
    ...
    sns.heatmap(
        ...
        square=True,
        linewidths=.5,
        cbar_kws={"shrink": .6, "label": "Cosine similarity ($S_C$)"},
        xticklabels=labels,
        yticklabels=labels,
        ax=ax
    )
